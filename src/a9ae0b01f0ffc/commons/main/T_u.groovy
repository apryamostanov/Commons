...
abstract class T_u {
    ...
    static final Boolean method_arguments_present(Object[] i_args) {
        return (i_args != T_const.GC_SKIPPED_ARGS && i_args != T_const.GC_NULL_OBJ_REF && i_args.size() > T_const.GC_EMPTY_SIZE)
    }

    static
    final String tokenize(T_static_string i_msg_text, String i_token_space, String i_token_trace, Object... i_traces = T_const.GC_SKIPPED_ARGS as Object[]) {
        Integer l_trace_seqno = 0
        String l_exception_text = i_msg_text.toString()
        l_exception_text = l_exception_text.replace(i_token_space, T_const.GC_SPACE)
    ...
    static final String process_location(String i_location) {
        ...
        l_location = l_location.replaceAll(T_s.commons().GC_SUBST_USERNAME, T_const.GC_USERNAME)
        ...
        l_location = l_location.replaceAll(T_s.commons().GC_SUBST_THREADID, T_const.GC_THREADID)
        l_location = l_location.replaceAll(T_s.commons().GC_SUBST_PROCESSID, T_const.GC_PROCESSID)
        return l_location
    }
    ...
    static final String bytes_to_string(byte[] i_bytes) {
        ...
        String l_hex_string = T_const.GC_EMPTY_STRING
        for (byte l_byte : i_bytes) {
            l_hex_string = Integer.toHexString(T_const.GC_FULL_BYTE & l_byte)
            l_result_string.append(l_hex_string.length() == T_const.GC_ONE_CHAR ? T_const.GC_ZERO + l_hex_string : l_hex_string)
        }
        return l_result_string.toString()
    }

    static final Object coalesce(Object... i_objects = T_const.GC_SKIPPED_ARGS) {
        if (method_arguments_present(i_objects)) {
            for (Object l_object : i_objects) {
                if (l_object != T_const.GC_NULL_OBJ_REF && l_object != T_const.GC_EMPTY_STRING) {
                    return l_object
                }
            }
            return T_const.GC_NULL_OBJ_REF
        } else {
            return T_const.GC_NULL_OBJ_REF
        }
    }
